        ...
        jQuery(function() {
         ...
         streetaddress, region, locality, postalcode, countryname, photo, workphone, workfax, homephone, cellphone, pager, url, bday, latitude, longitude, category, rev, uid, mailer) {
            ...
            function displayUI() {
                ...
                // if we have 1 mf and 1 map -- 580px px
                if (colColumnCount == 1 && (geoinfos.length > 0 || addresses.length > 0)) {
                    iwmf.css({ 'width': '580px' });
                    width = "580px";
                }

                // we have 2 mf and no map -- 580px px
            ...
            function findAndParseHCards() {
                ...
                oomph(hCardCollection).each(function() {
                    ...
                    //deal with case if they haven't typed their phone
                    var phone = null;
                    if (workphone == null && workfax == null && homephone == null && cellphone == null && pager == null)
                        phone = hCard.find('.tel').text();
                    ...
                    if (phone)
                        oomph(vcard).append('<div class="iwmf_phone">Phone: ' + phone + ' </div>');
            ...
            function phoneHelper(type, hCard) {
                ...
                //just grab the whole node identified by the tel attribute
                else {
                    selector = ".tel>.type=" + type;
                    //return hCard.find(selector).text();
                    return null;
                }

                //div.find("*:nth-child(0)").text();
